...
void SimpleStandardJobScheduler::initTaskPrioritySchemes() {
    ...
    this->task_selection_schemes["least_flops"] = [](const std::shared_ptr<wrench::WorkflowTask> &a,
                                                    const std::shared_ptr<wrench::WorkflowTask> &b) -> bool {
        if (a->getFlops() < b->getFlops()) {
            return true;
        } else if (a->getFlops() > b->getFlops()) {
    ...
    this->task_selection_schemes["least_data"] = [](const std::shared_ptr<wrench::WorkflowTask> &a,
                                                   const std::shared_ptr<wrench::WorkflowTask> &b) -> bool {
        double a_bytes = 0.0, b_bytes = 0.0;
        for (auto const &f : a->getInputFiles()) {
            a_bytes += f->getSize();
    ...
    this->task_selection_schemes["lowest_bottom_level"] = [this](
            const std::shared_ptr<wrench::WorkflowTask>& a,
            const std::shared_ptr<wrench::WorkflowTask>& b) -> bool {
    ...
    this->task_selection_schemes["least_children"] = [this](
            const std::shared_ptr<wrench::WorkflowTask>& a,
            const std::shared_ptr<wrench::WorkflowTask>& b) -> bool {
